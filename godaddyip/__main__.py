...
def main():
    ...
    parser_conf = command_parser.add_parser(
        "configure", help="Modify configuration file."
    )
    parser_conf.set_defaults(func=configure)
    parser_conf.add_argument(
        "config_file", type=Path, help="Location of the config file to modify."
    )
    conf_action_parser = parser_conf.add_subparsers()

    parser_conf_arecord = conf_action_parser.add_parser(
        "arecord", help="Update A record."
    )
    ...
    parser_conf_secret = conf_action_parser.add_parser(
        "secret", help="Update godaddy secret."
    )
    parser_conf_secret.set_defaults(action="set_secret")
    parser_conf_secret.add_argument("value")

    parser_conf_domain = conf_action_parser.add_parser(
        "domain", help="Update godaddy domain."
    )
    parser_conf_domain.set_defaults(action="set_domain")
    parser_conf_domain.add_argument("value")

    parser_conf_add = conf_action_parser.add_parser(
        "add-cname", help="Add or replace CNAME record."
    )
    parser_conf_add.set_defaults(action="add_cname")
    parser_conf_add.add_argument("value")

    parser_conf_del = conf_action_parser.add_parser(
        "del-cname", help="Remove CNAME record."
    )
